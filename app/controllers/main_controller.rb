...
class MainController < GenericController
  ...
  get '/:entity/:id' do
    ...
    if params.key?('depth')
      ...
      url = "#{data_logic_config[:host]}#{data_logic_config[:base_path].sub(/\/?$/, '/')}graph?from_cache=0&language=#{context.language}&entity=#{params[:entity].classify}&id=#{id}&depth=#{depth}"
      begin
